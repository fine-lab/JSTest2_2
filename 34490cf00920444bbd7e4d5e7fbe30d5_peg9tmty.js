...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    if (state == 1) {
      // 查询入库验收单子表
      let queryWarehouseSunSql = "select * from AT161E5DFA09D00001.AT161E5DFA09D00001.product_lis where WarehousingAcceptanceSheet_id = '" + id + "'";
      ...
      if (queryWarehouseSunList.length > 0) {
        for (let i = 0; i < queryWarehouseSunList.length; i++) {
          ...
          // 拼接查询条件
          let queryIf = " where product_umber = '" + registration_number + "' and productInformation_id = '" + product_code + "' order by product_certificate_date desc";
          // 查询产品信息档案
          let productSql =
            "select product_coding,id,the_product_name,unit,specifications,warehouse_storage_area_position_number_by_default,Entrusting_enterprise_name,registration_certificate_approval_date,enable,product_registration_number,whether_medical_equipment,registration,production_enterprise_code,registration_certificate_effective_date from AT161E5DFA09D00001.AT161E5DFA09D00001.productInformation where id = '" +
            product_code +
            "' order by pubts desc";
          let productList = ObjectStore.queryByYonQL(productSql);
          // 查询产品注册证
          let registrationSql =
            "select type_of_enterprise,nameRegistrant,storage_conditions,product_date,production_enterprise_code,production_enterprise_name,product_umber,product_certificate_date from AT161E5DFA09D00001.AT161E5DFA09D00001.product_registration_certifica" +
            queryIf;
          let registrationList = ObjectStore.queryByYonQL(registrationSql);
          // 货位号
          var warehouse_storage = "";
          let hwNO = includes(productList[0], "warehouse_storage_area_position_number_by_default");
          if (hwNO == false) {
            warehouse_storage = productList[0].warehouse_storage_area_position_number_by_default;
          }
          // 单位
          var unit = "";
          let unCN = includes(productList[0], "unit");
          if (unCN == false) {
            unit = productList[0].unit;
          }
          for (let s = 0; s < registrationList.length; s++) {
            // 备案人
            var nameRegistrant = "";
            let barCN = includes(registrationList[s], "nameRegistrant");
            if (barCN == false) {
              nameRegistrant = registrationList[s].nameRegistrant;
            }
            // 备案号
            var product_umber = "";
            let baNO = includes(registrationList[s], "product_umber");
            if (baNO == false) {
              product_umber = registrationList[s].product_umber;
            }
            // 储运条件
            var storage_conditions = "";
            let cyWhere = includes(registrationList[s], "storage_conditions");
            if (cyWhere == false) {
              storage_conditions = registrationList[s].storage_conditions;
            }
            // 生产企业id
            var productionCode = "";
            let proCode = includes(registrationList[s], "production_enterprise_code");
            if (proCode == false) {
              productionCode = registrationList[s].production_enterprise_code;
            }
            // 生产企业名称
            var production_enterprise_name = "";
            let proCN = includes(registrationList[s], "production_enterprise_name");
            if (proCN == false) {
              production_enterprise_name = registrationList[s].production_enterprise_name;
            }
          }
          SunData = {
            id: sunID,
      ...
      let registrationSql =
        ...
        product_code +
        "' and product_umber = '" +
        queryWarehouseSunList[0].registration_number +
