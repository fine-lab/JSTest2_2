    ...
    if (masterResult.length > 0) {
      if (masterResult[0].hasOwnProperty("ClientCode")) {
        ...
        for (var j = 0; j < childDataArr.length; j++) {
          ...
          // 获取质量状态
          // 获取每一条子表的校验状态
          var checkStatus = childDetail.checkStatus;
          // 判断校验状态
          // 已校验
          // 校验出库订单中产品的合法性
          // 获取出库订单中每一条子表的产品编码
          ...
          if (productResult.length > 0) {
            ...
            if (enable == 1) {
              // 获取产品信息的主表id
              var productionIds = productResult[0].id;
              // 查询产品信息子表
              // 根据产品名称，产品编码，注册证号查询产品注册证信息 and product_name = '"+Pcode+"'
              var sql =
              ...
              // 判断注册证返回结果
              if (proSonResult.length == 0) {
                arr.push("根据产品名称：'" + Pcode + "'，注册证号：'" + productRegisterNo + "'查询产品注册证信息失败，请检查！");
                continue;
              }
