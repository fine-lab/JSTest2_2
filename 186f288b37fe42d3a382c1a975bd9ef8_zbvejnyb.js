...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    var aeQuery = request.aelist;
    var AEArray = new Array();
    for (var i = 0; i < aeQuery.length; i++) {
      var rolename = aeQuery[i].name;
      let dqdata = rolename.substr(rolename.length - 2, rolename.length);
      AEArray.push(dqdata);
    }
    //获取token
    ...
    while (isdown) {
      ...
      if (rateresponseobj.code == "200") {
        ...
        for (var j = 0; j < queyData.length; j++) {
          var data = queyData[j];
          if (AEArray.indexOf(data.defines.define1_name) > -1) {
