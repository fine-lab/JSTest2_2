run = function (event) {
  ...
  viewModel.on("afterLoadData", function () {
    ...
    if (extend_is_gsp == "0" || extend_is_gsp == false) {
      if (viewModel.get("srcBillType").getValue() != "st_purchaseorder_return") {
        return;
      }
    }
    if (viewModel.getParams().mode == "add") {
      ...
      if (rows[0].qty > 0) {
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          promises.push(
            getSourceBillInfo(sourceBillId).then(
              (res) => {
        ...
        for (let i = 0; i < rows.length; i++) {
          let sourceBillId = rows[i].sourceid;
          promises.push(
            getSourceBillInfo_purOrder(sourceBillId).then(
              (res) => {
                if (!(res.extend_is_gsp == true || res.extend_is_gsp == "true" || res.extend_is_gsp == 1 || res.extend_is_gsp == "1")) {
                  return;
                }
                for (let j = 0; j < res.purchaseOrders.length; j++) {
                  if (rows[i].sourceautoid == res.purchaseOrders[j].id) {
                    //其他数量也要同时赋值
                    let qty = -(res.purchaseOrders[j].totalInQty + res.purchaseOrders[j].extend_review_qualified_qty);
                    //库存换算率
                    let invExchRate = rows[i].invExchRate;
                    //件数
                    let subQty = qty / invExchRate;
      ...
      Promise.all(promises).then(() => {
        ...
        if (errMsg.length > 0) {
          ...
          cb.utils.alert("查询到货单接口报错:" + errMsg, "error");
  ...
  viewModel.on("beforeSave", function () {
    ...
    //判断
    let validateStock = {
      inInvoiceOrg: viewModel.get("inInvoiceOrg").getValue(),
