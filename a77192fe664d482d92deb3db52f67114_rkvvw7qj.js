...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    var resxue = AppContext();
    var objss = JSON.parse(resxue);
    throw new Error(JSON.stringify(objss));
    var datass = objss.currentUser;
    var useridss = datass.id;
    let sqlsti = "select * from base.user.user   where yhtUserId = '" + datass + "'";
    var resdatastiss = ObjectStore.queryByYonQL(sqlsti, "productcenter");
    throw new Error(JSON.stringify(resdatastiss));
    ...
    orderDetails.forEach((dataod) => {
      if (dataod._selected == true) {
        ...
        if (totalsummary.length > 0) {
          ...
          for (var i = 0; i < b; i++) {
            if (totalsummary[i] == dataod.headDefine.define3_name) {
              ...
              //修改总表的数据
              //总运费要变
              titleArray[i].total_freight = titleArray[i].total_freight + dataod.defines.define4;
              //总数量也要变
              titleArray[i].total_num = titleArray[i].total_num + dataod.qty / 1000;
        ...
        if (biaoji == 0) {
          ...
          try {
            var define3 = dataod.defines.define3_name;
          } catch (e) {
          ...
          var objectlist = {
            ...
            //物流公司
            materialcompany_name: dataod.defines.define3_name,
            materialcompany: dataod.defines.define3,
            //组织id
            ordid: userid,
            //原始单据类型
            ...
            state: "已审核",
            //总运费
            total_freight: dataod.defines.define4,
            //总数量
          ...
          totalsummary.push(dataod.defines.define3_name);
