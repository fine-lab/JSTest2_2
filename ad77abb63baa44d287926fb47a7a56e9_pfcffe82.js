  ...
  execute(context, param) {
    ...
    for (let i = 0; i < bills.length; i++) {
      ...
      if (deletePlans && deletePlans.length > 0) {
        let deleteRes = ObjectStore.deleteBatch("GT7139AT4.GT7139AT4.sy_annualplan", deletePlans);
      }
      if (updatePlans && updatePlans.length > 0) {
        let updateRes = ObjectStore.updateBatch("GT7139AT4.GT7139AT4.sy_annualplan", updatePlans);
      }
    }
    return {};
  }
  resetPlanInfo(plan, lastPlan) {
    let needReFields = ["variableCost", "internalPrice", "purchaseNum", "purchaseAmount", "purchaseMargin", "confirmDate", "confirmRatio", "monthlyRatio"];
    let resetPlan = {};
    for (let key of needReFields) {
      resetPlan[key] = lastPlan[key];
    }
