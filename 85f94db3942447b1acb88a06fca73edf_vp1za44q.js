...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    if (datasAll.hasOwnProperty("样本编号*") != true) {
      ...
      throw new Error(err);
    } else if (datasAll.hasOwnProperty("DNA样本类型*") != true) {
    ...
    if (
      ...
      throw new Error("  -- DNA样本类型输入有误,请重新输入 --  ");
    }
    //枚举
    var DNAYBLX = datasAll["DNA产品类型"];
    if (DNAYBLX != null) {
      if (DNAYBLX != "1" && DNAYBLX != "2" && DNAYBLX != "3" && DNAYBLX != "4") {
        throw new Error("  -- DNA产品类型输入有误,请重新输入 --  ");
      }
    }
    //枚举
    ...
    //枚举
    var DNASYLC = datasAll["DNA实验流程"];
    if (DNASYLC != null) {
      if (DNASYLC != "1" && DNASYLC != "2" && DNASYLC != "3") {
        throw new Error("  -- DNA实验流程输入有误,请重新输入 --  ");
      }
    }
    ...
    var object = {
      ...
      dnaCPType: datasAll["DNA产品类型"],
      dnaYBType: datasAll["DNA样本类型*"],
