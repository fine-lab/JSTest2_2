function init(event) {
  ...
  viewModel.get("button6600ga").on("click", function (event) {
    //获取新增子类时的行数据
    ...
    let resp = cb.rest.invokeFunction("696bb84e333546d09cd779d5c8e79feb", { qualificationClassification: parentRow.id }, null, viewModel, { async: false });
  ...
  function enableStatus(event) {
    ...
    if (names.length === 0) {
      ...
      let selectResult = cb.rest.invokeFunction("3240b8d277024ebfbfbfad482f1a252b", { names: names }, null, viewModel, { async: false });
      var ids = [];
      ...
      for (var m in ids) {
        ...
        let respResult = cb.rest.invokeFunction("0947ee7cc82a4c878ca7d0ccbd350b1f", { data: data }, null, viewModel, { async: false });
        var s = 1;
      }
