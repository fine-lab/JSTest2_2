...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    const sourcecode = datas.code; //单据编码
    const remark = datas.memo; //备注
    const flzc = datas["headItem!define1_name"]; //返利政策
    const flzcID = datas["headItem!define1"]; //返利政策 ID
    const zhsygz = datas.shareSettingId_name; //账户使用规则
    const kpzz = datas.settlementOrgId_name; //开票组织
    const kpzzID = datas.settlementOrgId; //开票组织 ID
    const xszz = datas.salesOrgId; //销售组织 ID
