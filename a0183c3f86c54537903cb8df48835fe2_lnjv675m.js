...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    var access_token = getToken(config.appKey, config.appSecrect);
    // 获取token方法
    ...
    function postdatanotoken(url, body) {
      const header = {
        "Content-Type": "application/json"
      };
      var list_body_strResponse = postman("post", url, JSON.stringify(header), JSON.stringify(body));
      var Response = JSON.parse(list_body_strResponse);
      return Response;
    }
    function getuserinfo(scope, scopeCode) {
      var url = "https://www.example.com/";
      var body = {
        scope: scope,
        scopeCode: scopeCode
      };
      var accept = postdatawithtoken(access_token, url, body);
      return accept;
    }
    var accept = getuserinfo(config.appCode, "杨奕峰");
    return { request: param, accept: accept };
  }
}
