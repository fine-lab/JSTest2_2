...
viewModel.get("button11vg").on("click", function (args) {
  ...
  // 缓存数据
  cb.cache.set("plan_cache_" + learning_plan_id, plan_data);
...
viewModel.on("customInit", function (data) {
  ...
  function getRate() {
    ...
    if (isReplace) {
      var my_plan_cache = cb.cache.get("plan_cache_" + viewModel.getData().id);
      if (my_plan_cache) {
        console.log("===获取缓存数据===");
        ...
        viewModel.get("total_rate").setValue(my_plan_cache.total_rate);
        console.log("======替换表格========");
  ...
  viewModel.on("afterLoadData", function (data) {
    data = plan_data.learning_task1List;
    viewModel.get("total_rate").setValue(plan_data.total_rate);
    gridModel.setState("dataSourceMode", "local");
