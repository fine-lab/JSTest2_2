      ...
      if (cgdhdResbody.length > 0) {
        ...
        if (cgddResbody.length > 0) {
          ...
          if (qgdResboy.length > 0) {
            ...
            //查询请购单子表自由自定义数据
            let queryQGDZDYbody = "select * from pu.applyorder.ApplyOrdersFreeItem where id=" + applyOrders.id;
            let qgdzdyResboy = ObjectStore.queryByYonQL(queryQGDZDYbody, "upu");
            if (qgdzdyResboy.length > 0) {
              let qgdzdyData = qgdzdyResboy[0];
              //查询销售预订单子表
              let queryXSYDDbody = "select * from GT83441AT1.GT83441AT1.salesAdvanceOrder_b where id=" + qgdzdyData.define3;
