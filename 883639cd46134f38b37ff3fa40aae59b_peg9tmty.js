  ...
  if (args.params.cCommand == "cmdBatchUnstop" || args.params.cCommand == "cmdBatchStop") {
    ...
    data.forEach((item) => {
      if (item.enable == 1 && args.params.cCommand == "cmdBatchUnstop") {
        errArr.push({ dict_code: item.dict_code, enable: "启用" });
      }
      if (item.enable == 0 && args.params.cCommand == "cmdBatchStop") {
        errArr.push({ dict_code: item.dict_code, enable: "停用" });
      }
    });
    if (errArr.length > 0) {
      var errMsg = "";
      for (var index = 0; index < errArr.length; index++) {
        errMsg += "代码值: " + errArr[index].dict_code + "，单据已经属于" + errArr[index].enable + "状态\n";
      }
...
viewModel.on("beforeBatchdelete", function (args) {
  ...
  data.forEach((item) => {
    ids.push(item.id);
    if (item.enable == 1) {
      errArr.push(item.dict_code);
    }
  });
  if (errArr.length > 0) {
    var errMsg = "删除失败：代码值: " + errArr.join("/") + "，为启用状态不可删除";
    cb.utils.alert(errMsg, "error");
    return false;
  }
  cb.rest.invokeFunction("AT161E5DFA09D00001.apiFunction.rangeIfQuote", { ids: ids }, function (err, res) {
    console.log(res);
    if (res && res.exist) {
      ...
      cb.utils.confirm(
        "确定删除吗?",
        () => {
          cb.rest.invokeFunction("AT161E5DFA09D00001.apiFunction.rangeDelete", { ids: ids }, function (err, res) {
