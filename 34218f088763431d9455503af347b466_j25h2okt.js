...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    if (InAfterSave == "Update") {
      ...
      let fun = extrequire("ST.unit.publicInDelete");
      let orderItemData = fun.execute(null, orderItem);
      console.log(JSON.stringify(orderItemData));
      let header = { "Content-Type": "application/json;charset=UTF-8" };
      let strResponse = postman("post", "http://47.100.73.161:888/api/unified", JSON.stringify(header), JSON.stringify(orderItemData.orderBody));
