    ...
    if (res.length == 0) {
      ...
      if (resOne.length != 0) {
        ...
        for (var i = 0; i < resOne.length; i++) {
          //主表id   where id ='"+zid+"'
          var zid = resOne[i].DetectOrder_id;
          var sqlTwo = "select * from AT15F164F008080007.AT15F164F008080007.DetectOrder where id = '" + zid + "'";
          var resTwo = ObjectStore.queryByYonQL(sqlTwo);
          var zijian = resTwo[0].InspectionForm;
          if (zijian == "01") {
            var sid = resOne[i].id;
            var object = { id: zid, BOMImportList: [{ id: sid, caigoudingdanhao: "", _status: "Update" }] };
