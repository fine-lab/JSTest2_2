...
viewModel.on("afterMount", () => {
  ...
  if (btjmx == undefined) {
    cb.utils.alert("获取主子切换失败：" + btjmx);
  }
  if (btjmx == false) {
    viewModel.get("button211qj").setVisible(false); //下推采购订单(项目委外)
    ...
    //表头
    viewModel.get("button211qj").setVisible(true); //下推采购订单(项目委外)
...
viewModel.get("button139th") &&
  viewModel.get("button139th").on("click", function (data) {
    ...
    cb.rest.invokeFunction("AT15F164F008080007.jcdd.implementCBJS", { importData: monthData }, function (err, res) {}, viewModel, { async: false });
...
function execlponsereport(workbookDatas) {
  ...
  var voucherResponse = cb.rest.invokeFunction("AT15F164F008080007.jcdd.reportImport", { reportData: sheetone, nowTime: monthData }, function (err, res) {}, viewModel, { async: false });
  document.getElementById("filee_input_info").value = "";
...
viewModel.get("button203hg") &&
  viewModel.get("button203hg").on("click", function (data) {
    ...
    var voucherResponse = cb.rest.invokeFunction("AT15F164F008080007.jcdd.MaterialIssue", { selectDatas: selectDatas, nowTime: monthData }, function (err, res) {}, viewModel, { async: false });
    cb.utils.alert("后台进入执行，请等待", "success");
  });
