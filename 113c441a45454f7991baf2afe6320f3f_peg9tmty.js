  ...
  execute(request) {
    ...
    if (clientCodeRes.length == 0) {
      ...
      if (clientList.length != 0) {
        ...
        return { err: "出库单" + ClientCode + "委托方企业信息不存在或已停用，请维护委托方企业信息再进行导入" };
      }
      ...
      for (var i = 0; i < request.importSubtable.length; i++) {
        ...
        if (res.length != 0) {
          ...
          //查询产品注册证
          var objectRes = { productInformation_id: sonId, product_umber: product_registration_number };
          var resSonStr = ObjectStore.selectByMap("AT161E5DFA09D00001.AT161E5DFA09D00001.product_registration_certifica", objectRes);
          if (resSonStr.length != 0) {
            enterpriseName = resSonStr[0].production_enterprise_name;
            productionName = resSonStr[0].production_enterprise_code;
            nameRegistrant = resSonStr[0].nameRegistrant;
            conditionsName = resSonStr[0].storage_conditions;
          }
        } else {
          //实体查询库存
          let body = {
          ...
          if (resSonStr.length != 0) {
            ...
            nameRegistrant = resSonStr[0].nameRegistrant;
            conditionsName = resSonStr[0].storage_conditions;
            var UI = resSonStr[0].ui;
            var DI = resSonStr[0].di;
            var UDI = resSonStr[0].udi;
          }
        }
