viewModel.get("ytenant") &&
  ...
  viewModel.on("afterLoadData", function (data) {
    ...
    if (ytenant === buyerTenant && viewModel.get("detailList") && viewModel.get("detailList").getEditRowModel().get("detailFile")) {
      viewModel.get("detailList").setColumnState("price", "visible", false);
      viewModel.get("detailList").setColumnState("noTaxPrice", "visible", false);
    } else {
    }
    //新增清空金额数据
    if ("add" === viewModel.originalParams.mode || "edit" === viewModel.originalParams.mode) {
      let rows = viewModel.get("detailList").getRows();
      for (let i = 0; i < rows.length; i++) {
        let row = rows[i];
        viewModel.get("detailList").setCellValue(i, "price", null);
        viewModel.get("detailList").setCellValue(i, "noTaxPrice", null);
        viewModel.get("detailList").setCellValue(i, "totalPrice", null);
        viewModel.get("detailList").setCellValue(i, "noTaxMoney", null);
      }
    }
    viewModel.get("detailList").setColumnState("totalPrice", "bCanModify", true);
  });
viewModel.get("detailList") &&
