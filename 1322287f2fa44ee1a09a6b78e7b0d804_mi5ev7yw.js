viewModel.on("customInit", function (data) {
  ...
  let updateProLics = function (materialId, isBatchManage, isExpiryDateManage, expireDateNo, expireDateUnit, producingArea, modelDescription, unit, unitName, isExpiryDateCalculationMethod) {
  ...
  let getFirstStatus = function (customerbillno) {
    return new Promise((resolve, reject) => {
      invokeFunction1(
        "GT22176AT10.sygl.getFirstStatus",
        {
  ...
  viewModel.on("afterLoadData", function (data) {
    ...
    if (currentState != "add") {
      let customerbillno = viewModel.get("code").getValue();
      let returnPromise = new cb.promise();
      getFirstStatus(customerbillno).then(
        (res) => {
  ...
  viewModel.on("beforeSave", function () {
    ...
    if (currentState != "add") {
      ...
      let producingArea = viewModel.get("placeOfOrigin").getValue();
      let modelDescription = viewModel.get("modelDescription").getValue();
      if (modelDescription != undefined) {
        modelDescription = modelDescription.zh_CN;
      }
      let unit = viewModel.get("unit").getValue();
      let unitName = viewModel.get("unit_Name").getValue();
      let isExpiryDateCalculationMethod = viewModel.get("detail!isExpiryDateCalculationMethod").getValue();
      let returnPromise = new cb.promise();
      updateProLics(materialId, isBatchManage, isExpiryDateManage, expireDateNo, expireDateUnit, producingArea, modelDescription, unit, unitName, isExpiryDateCalculationMethod).then(
        (res) => {
          returnPromise.resolve();
        },
        (err) => {
  ...
  let getFirstProductInfo = function (materialId) {
    return new Promise((resolve, reject) => {
      invokeFunction1(
        "GT22176AT10.backDefaultGroup.getFirstProductInfo",
        {
          materialId: materialId,
          type: "delete"
        },
  ...
  viewModel.on("beforeDelete", function (data) {
    ...
    let sqwtsChildRes = [];
    promiseArr.push(
      getFirstProductInfo(materialId).then((res) => {
        sqwtsChildRes = res;
      })
    );
    let returnPromise = new cb.promise();
    Promise.all(promiseArr).then((res) => {
      if (sqwtsChildRes.length > 0) {
        cb.utils.alert(sqwtsChildRes, "error");
        returnPromise.reject();
      } else {
        returnPromise.resolve();
      }
  ...
  function addMaterialApply(orgId, orgName, materialId, productCode, productName, mnemonicCode) {
    return new Promise((resolve, reject) => {
      invokeFunction1(
        "ISY_2.backOpenApiFunction.addMaterialApply",
        {
          orgId: orgId,
          orgName: orgName,
          materialId: materialId,
