...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    var startTime = day2.getFullYear() + "-" + addZero(day2.getMonth() + 1 + "") + "-" + addZero(day2.getDate() + "");
    var day3 = new Date();
    day3.setTime(day3.getTime() + 24 * 60 * 60 * 1000);
    var endTime = day3.getFullYear() + "-" + addZero(day3.getMonth() + 1 + "") + "-" + addZero(day3.getDate() + "");
    //需要处理部分------------------------begin
    var hmd_req_resources_path = "/homedo-yonsuite-service/getProduct?startTime=2020-01-01&endTime=2020-02-20";
    //注意需要大写
    ...
    var reqData = JSON.parse(apiResponse).data.data;
    if (reqData != null) {
      if (reqData.onshelves.length > 0) {
        var res = ObjectStore.insertBatch("GT1281AT3.GT1281AT3.goods", resultToSqlUseParam(reqData.onshelves, "1"), "d0d02c28");
      }
      if (reqData.offshelves.length > 0) {
        var res = ObjectStore.insertBatch("GT1281AT3.GT1281AT3.goods", resultToSqlUseParam(reqData.onshelves, "2"), "d0d02c28");
      }
      if (reqData.onshelves.length == 0 && reqData.offshelves.length == 0) {
        throw new Error("没有拉取到数据哦!");
      }
    }
    //数据转换-----------------------begin
    function resultToSqlUseParam(data, isUp) {
      data.map((v) => {
        data.goodstate = isUp;
      });
      return data;
