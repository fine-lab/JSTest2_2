...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    if (masterRes.length > 0) {
      for (let i = 0; i < masterRes.length; i++) {
        ...
        if (request.type == "证照范围") {
          ...
          licenceChildRes = ObjectStore.selectByMap("ISY_2.ISY_2.SY01_supply_licence", licenceChildObject);
        ...
        if (request.type == "授权委托书范围") {
          ...
          sqwtsChildRes = ObjectStore.selectByMap("ISY_2.ISY_2.SY01_supply_power_attorney", sqwtsChildObject);
    ...
    let object = { proId: proId, skuId: skuId, licenceChildRes: licenceChildRes, sqwtsChildRes: sqwtsChildRes };
    return { object };
  }
}
