        ...
        if (data_v1.godown_type_v1 != undefined && data_v1.production_condition_name != undefined && data_v1.chanpinpanduan_v1 != undefined) {
          ...
          if (true) {
            ...
            for (var i = 0; i < alllength_data.length; i++) {
              ...
              if (alllength_data[i].weigth == undefined) {
                continue;
              }
              weigthAllAdd = weigthAllAdd + parseFloat(alllength_data[i].weigth);
            }
            parentViewModel.get("summary_length").setValue(lengthAllAdd);
            parentViewModel.get("summary_weight").setValue(weigthAllAdd.toFixed(2));
          }
          //汇总重量
          ...
          //设置废品汇总数量
          parentViewModel.get("waste_quantity_v1").setValue(parseFloat(waste_quantity.toFixed(2)));
...
viewModel.get("chanpinpanduan_v1") &&
  viewModel.get("chanpinpanduan_v1").on("afterValueChange", function (data) {
    ...
    if (data.value.text == "一级管") {
      viewModel.get("production_condition").setValue("1760269213732700234"); //生产状况id
