viewModel.get("BasicInformationDetailsList") &&
  viewModel.get("BasicInformationDetailsList").on("afterCellValueChange", function (data) {
    ...
    // 获取进场日期
    var MobilizationDate = BasicInformationDetailsList.getCellValue(Index, "jinchangriqi");
    // 获取完工袋完成日期
    var CompletionDate = BasicInformationDetailsList.getCellValue(Index, "shijiyanshouriqi");
    ...
    let JS = 0;
    //判断上二排日期是否为空
    if (BasicInformationDetailsList.getCellValue(Index, "shangerpairiqi") != null) {
      DP = DP + 1;
      //获取上二排日期
      var SRP = BasicInformationDetailsList.getCellValue(Index, "shangerpairiqi");
      //获取上二次地盘检查报告日期
      var ERP = BasicInformationDetailsList.getCellValue(Index, "ercidipanjianchabaogao");
      if (ERP != null) {
        DP = DP + 1;
    ...
    //判断日立发货日期是否为空
    if (BasicInformationDetailsList.getCellValue(Index, "rilifahuoriqi") != null) {
      FH = FH + 1;
      //获取日立发货日期
      var RLDate = BasicInformationDetailsList.getCellValue(Index, "rilifahuoriqi");
      //获取任务下达单提交日期
      var RXDate = BasicInformationDetailsList.getCellValue(Index, "renwuxiadadantijiaoriqi");
      //获取客户施工计划日期
      var KHDate = BasicInformationDetailsList.getCellValue(Index, "kehushigongjihua");
      //获取告知日期
      var GZDate = BasicInformationDetailsList.getCellValue(Index, "gaozhiriqi");
    ...
    if (viewModel.get("Acceptance_date").__data.value != null) {
      ...
      //获取监理微信群日期
      var JLDate = BasicInformationDetailsList.getCellValue(Index, "jianliweixinqun");
      //获取一次地盘检查报告日期
      var YCDate = BasicInformationDetailsList.getCellValue(Index, "yicidipanjianchabaogao");
      //获取现场检查照片日期
      var XCDate = BasicInformationDetailsList.getCellValue(Index, "xianchangjianchazhaopian");
      //获取温馨提示日期
      var WXDate = BasicInformationDetailsList.getCellValue(Index, "wenxintishi");
      //获取报装资料提示日期
      var BZDate = BasicInformationDetailsList.getCellValue(Index, "baozhuangziliaotishi");
    ...
    //回写地盘报告2状态
    BasicInformationDetailsList.setCellValue(Index, "dipanbaogao2zhuangtai", DP, true);
    //回写发货五天内状态
    BasicInformationDetailsList.setCellValue(Index, "fahuo5tiannazhuangtai", FH, true);
    //回写接收合同30天状态
    BasicInformationDetailsList.setCellValue(Index, "getState", JS, true);
  });