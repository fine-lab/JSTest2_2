...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    let bills = request.data;
    // 根据id查询明细
    let searchSQL =
      "select *,org_id.name,(select *,unit.name,(select * from shippingschedulesnList) as suppliershippingschedulesnList from shippingschedulebList) suppliershippingschedulebList from GT37595AT2.GT37595AT2.shippingschedule " +
      request.whereStr;
    ...
    let tokenSql = "select vendorId,appKey,appSecret,appTenantId from GT39745AT6.GT39745AT6.suplierApiMessage " + request.whereSuplier.replace("where id in", "where vendorId in");
    ...
    for (var i = 0; i < res.length; i++) {
      var bill = res[i];
      var relationTenantIdItem = tokenMap.get(bill.vendorId).appTenantId;
      bill["source_id"] = bill.id;
      bill["purchaser_tenant_id"] = tenantId;
      bill["purchaser_tenant_name"] = tenantName;
      bill["tenant_id"] = relationTenantIdItem;
      bill["submit_status"] = "2"; // 未提交
      bill["submit_status_name"] = "未提交"; // 未提交
      bill["agentName"] = bill.org_id_name;
      for (var j = 0; j < bill.suppliershippingschedulebList.length; j++) {
        var shippingscheduleb = bill.suppliershippingschedulebList[j];
        shippingscheduleb["source_id"] = shippingscheduleb.id;
        shippingscheduleb["tenant_id"] = relationTenantIdItem;
        shippingscheduleb["unitName"] = shippingscheduleb.unit_name;
        if (shippingscheduleb.suppliershippingschedulesnList && shippingscheduleb.suppliershippingschedulesnList.length > 0) {
          for (var z = 0; z < shippingscheduleb.suppliershippingschedulesnList.length; z++) {
            var shippingschedulesn = shippingscheduleb.suppliershippingschedulesnList[z];
            shippingschedulesn["source_id"] = shippingschedulesn.id;
            shippingschedulesn["tenant_id"] = relationTenantIdItem;
          }
        }
      }
      var billData = { data: [bill] };
      //使用-传参分别为： appkey, appsecrect
      let header = { appkey: tokenMap.get(bill.vendorId).appKey, appsecret: tokenMap.get(bill.vendorId).appSecret };
      let url = configParams.gatewayUrl + "/" + relationTenantIdItem + "/product_ref/product_ref_01/batchSaveSupplierBill";
      let apiResponse = ublinker("post", url, JSON.stringify(header), JSON.stringify(billData));
      rrr = apiResponse;
    }
    // 更新主表状态，防止重复提交
    var returnRes = ObjectStore.updateBatch("GT37595AT2.GT37595AT2.shippingschedule", bills, "shippingschedule");
    return { rrr };
    //获取token方法
