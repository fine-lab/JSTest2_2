...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    var hmd_req_resources_path = "/homedo-yonsuite-service/sureOrder";
    //注意需要大写
    ...
    ybresult.forEach((data) => {
      ...
      object.isconfirm = "1";
      object.id = data.id;
      let res = ObjectStore.updateById("GT1281AT3.GT1281AT3.receive", object);
    }); /*
     var body={ 'orderId':'101105980'};
     let body1=JSON.stringify(body);
     let head=JSON.stringify(header);
     let uri=(hmd_base_path).concat(hmd_req_resources_path);
     //请求数据
     let apiResponse = postman("post", uri
     ,head,body1);
    //数据转换-----------------------end
