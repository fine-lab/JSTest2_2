...
viewModel.on("afterProcessWorkflow", function (args) {
  let ZGgrid = viewModel.getGridModel("ZGList");
  let theContainerType = viewModel.get("TheContainerType").getValue();
  let bShowIt = viewModel.get("ZGList").getColumn("shifutongyizhuangguifanganzg").bShowIt;
  debugger;
  if (theContainerType == "1" && bShowIt) {
    viewModel.get("ZGList").setColumnState("shifutongyizhuangguifanganzg", "bIsNull", false);
    viewModel.get("ZGList").setColumnState("zhuangguifanganzg", "bIsNull", false);
  } else {
    viewModel.get("ZGList").setColumnState("shifutongyizhuangguifanganzg", "bIsNull", true);
    viewModel.get("ZGList").setColumnState("zhuangguifanganzg", "bIsNull", true);
  }
  let PXgrid = viewModel.getGridModel("PXList");
  let bShowItPx = viewModel.get("PXList").getColumn("shifutongyizhuangguifangan").bShowIt;
  if (theContainerType == "2" && bShowItPx) {
    viewModel.get("PXList").setColumnState("shifutongyizhuangguifangan", "bIsNull", false);
    viewModel.get("PXList").setColumnState("zhuangguifangan", "bIsNull", false);
  } else {
    viewModel.get("PXList").setColumnState("shifutongyizhuangguifangan", "bIsNull", true);
    viewModel.get("PXList").setColumnState("zhuangguifangan", "bIsNull", true);
  }
});