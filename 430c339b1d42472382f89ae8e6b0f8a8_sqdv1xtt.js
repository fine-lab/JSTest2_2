  ...
  gridModel.on("afterSetDataSource", () => {
    ...
    rows.forEach((data) => {
      ...
      actions.forEach((action) => {
        ...
        if (action.cItemName == "B_singlepushba995383-ccd5-11ec-9896-6c92bf477043" || action.cItemName == "B_singlepush8d515d72-ccfe-11ec-9896-6c92bf4770433") {
          if (data.status != 1) {
        ...
        if (typeof res !== "undefined") {
          for (let i = 0; i < res.entry.length; i++) {
            if (res.entry[i].acceptqty - res.entry[i].extend_associate_sample_qty <= 0) {
              message += res.code + "的第" + (i + 1) + "实收数量 - 累计检验数量<=0,无法下推\n";
            }
          }
        } else if (err !== null) {
          message = err.message;
