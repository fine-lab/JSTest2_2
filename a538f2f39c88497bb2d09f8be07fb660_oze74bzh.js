...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    request.appId = appId;
    let config = configfun.execute(request).config;
    throw new Error("token config" + JSON.stringify(config));
    var access_token = getToken(config.appkey, config.appSecret, config.baseApiUrl);
    //获取token方法
