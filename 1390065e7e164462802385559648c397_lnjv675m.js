viewModel.on("customInit", function (data) {
    ...
    apipost(params, vbody, options, action).then((res, err) => {
      if (res) {
        ...
        if (status == 0) {
          ...
          saveGxsuser(userId, staffRes.SysStaffNew, staffRes.UserMobile.substring(4, 15), staffRes.id, staffRes.AgentStaffCode, staffRes).then((res, err) => {
            if (res) {
              returnPromise.resolve(res);
            } else {
              returnPromise.reject(err);
  ...
  function saveGxsuser(yhtuid, staff, phone, gxsstaff, gxsstaffcode, staffRes) {
    ...
    cb.rest.invokeFunction("GT34544AT7.user.searchUserInfoByUid", { id: yhtuid }, function (err, res) {
      ...
      getUserinfo(phone).then((info, err) => {
        ...
        console.log(uinfo);
        if (!!uinfo) {
          delUserBind(uinfo).then((res, err) => {
            console.log("解绑用户返回消息:");
            console.log(JSON.stringify(res));
            ...
            binduser(gxsstaffcode, userId, userCode, yhtuid, mystaff).then((res, err) => {
              ...
              if (res) {
                ...
                changeSqlIsUser(mystaff2, uidx, userCode, uinfo.yhtUserId).then((res, err) => {
                  if (!!res) {
                    console.log("AgentrUser修改后返回");
                    console.log(res);
                    saveUserInfo(staffRes.AgentStaff, gxsstaff).then((res, err) => {
                      if (!!res) {
                        returnPromise.resolve(res);
                      } else {
                        returnPromise.reject(err);
          ...
          binduser(gxsstaffcode, userId, userCode, yhtuid, mystaff).then((res, err) => {
            ...
            if (res) {
              console.log(res);
              changeSqlIsUser(mystaff2, userId, userCode, yhtuid).then((res, err) => {
                if (!!res) {
                  console.log("AgentrUser修改后返回");
                  console.log(res);
                  saveUserInfo(staffRes.AgentStaff, gxsstaff).then((res, err) => {
  ...
  // 同步用户信息
  function saveUserInfo(AgentStaff, AgentUser) {
    let returnPromise = new cb.promise();
    let obj = {
      id: AgentStaff,
      agentUser: AgentUser
    ...
    cb.rest.invokeFunction(
      ...
      {
        table: "GT1559AT25.GT1559AT25.AgentStaff",
        object: obj,
        billNum: "579f1ffe"
      },
      function (err, res) {
  ...
  function changeSqlIsUser(mystaff, userId, userCode, yhtuid) {
    ...
    cb.rest.invokeFunction(
      "GT34544AT7.common.updatesql",
      {
        table: "GT1559AT25.GT1559AT25.AgentUser",
        object: obj,
        billNum: "893bc133"
      },
      function (err, res) {
  ...
  function checkAgentOrg(id) {
    ...
    cb.rest.invokeFunction(
      "GT34544AT7.common.selectSqlByMapX",
      {
        table: "GT1559AT25.GT1559AT25.AgentOrg",
        conditions: obj
      },
      function (err, res) {
        if (res) {
  ...
  viewModel.on("afterSave", function (data) {
    console.log("保存成功");
    //自建员工对象
    let staffRes = data.res;
    let AgentStaff = staffRes.AgentStaff;
    if (!!AgentStaff) {
      // 添加用户
      addSysUser(staffRes).then((res, err) => {
        if (res) {
          console.log(res);
        }
      });
    } else {
      console.log("没有代账用户参数");
    }
  });
});